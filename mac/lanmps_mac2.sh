...
cp -rf redis.sh $IN_DIR/redis.sh

chmod +x $IN_DIR/nginx
chmod +x $IN_DIR/redis.sh


sed -i "" s/fox/${IN_USER}/g $IN_DIR/vhost/default.conf
sed -i "" s/fox/${IN_USER}/g $IN_DIR/nginx
sed -i "" s/fox/${IN_USER}/g /usr/local/etc/nginx/nginx.conf
sed -i "" 's/use epoll;/#use epoll;/g' /usr/local/etc/nginx/nginx.conf
...
sed -i "" 's:;request_terminate_timeout = 0:request_terminate_timeout = 100:g' /usr/local/sbin/php56-fpm
