...
PHP_PATH=${IN_DIR}/php${VERS['php5.4.x']}
tmp_configure=""
...
rm -rf "/usr/bin/php-fpm54"

ln -s "${PHP_PATH}/bin/php" /usr/bin/php54
ln -s "${PHP_PATH}/bin/phpize" /usr/bin/phpize54
...
if [ $SERVER == "nginx" ]; then
        ...
        cp "${IN_DOWN}/php-${PHP_VER}/sapi/fpm/init.d.php-fpm" $IN_DIR/bin/php-fpm54
        chmod +x $IN_DIR/bin/php-fpm54
